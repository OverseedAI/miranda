...
export async function runCrawlCycle(): Promise<{
  ...
  try {
    ...
    for (const rawArticle of newArticles) {
      console.log(
        `[Scheduler] Analyzing: ${rawArticle.title.slice(0, 50)}...`
      );
      ...
      if (
        analysis.isVideoWorthy &&
        analysis.score >= config.videoWorthyThreshold
      ) {
        stats.videoWorthy++;
        console.log(
...
export function startCrawlScheduler(): void {
  console.log(
    `[Scheduler] Starting with ${config.crawlIntervalMs / 1000 / 60 / 60}h interval`
  );
